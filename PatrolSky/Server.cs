...
namespace PatrolSky
{
    class Server
    {
        ...
        public static void start(int _port) //   서버 시작
        {
            ...
            {
                broadcastServer = new Server(_port);
            }
        }
        ...
        private void WaitingSocket()    //  클라이언트의 접속을 체크하는 스레드.
        {
            ...
            while (true)
            {
                ...
                UIManager.Instacne.form.대기유저추가(socketList.Count + "번째 컴퓨터 - " + acceptedIP + "에서 접속하였습니다.");
    ...
    public class SocketHandler
    {
        ...
        public void Handler()                   //  실질적인 서버작업
        {
            ...
            int bufferCount;
            SendMsg((Server.socketList.Count) + "명이 접속해 있습니다.");
            try
            {
                while (true)
                {
                    ...
                    UIManager.Instacne.form.서버상태업데이트(Msgs);
                   
                    //SendMsg(Msgs); 메시지를 받는 부분이다.
                }
            }
            catch (Exception e)
            {
